...
spec_input[,'fluxerr'] = spec_input[,'flux']*0.05
```
...
plot(inputs)
plot(inputs$SEDout, xlim=c(1e3,2e6), ylim=c(1e3,5e6))
```
...
Highout = Highlander(badpar, Data=Data, likefunc=ProSpectSEDlike, Niters=c(1e2,1e2), optim_iters=5)
```
...
plot(outputs)
plot(outputs$SEDout, xlim=c(1e3,2e6), ylim=c(1e3,5e6))
```
