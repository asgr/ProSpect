...
Where relevant I give link you can type into the R console with the package loaded to have a look at soemthing in more detail, these start with a '?'.
...
For most uses getting up and running, having a play with the simplest 4 phase burst star formation (**SFHp4**) is probably the best way to start. Have a read of the documentation and Examples in ?SFHp4. Fitting data is a more complex affair, but users can get a feel for how this works in practice by looking at ?SFHp4like. The basic idea of these likelihood functions is that they allow the user to provide observed Data and can marginalise over various types of SFH and dust models in order to constrain a set of target parameters. It is also a good idea to get acquainted with the main vignettes covering the BC03 and EMILES data compiled into the package (SSPchecks and Comparisons). These give a good idea how to access the data at a low-level, and allows for easy manipulation and potentially user defined functions for processing them.

The 4 phase model (**SFHp4**) covers 4 key phases of star formation and models them as having constant star formation in each of these periods. By default they cover 0 - 100 Myr (burst, generally considered the shortest phase that broad band photometry can be sensitive to); 100 Myr - 1 Gyr (young; dominated by hot young stars and violent phases of stellar evolution); 1 Gyr - 9 Gyr (old) and 9 Gyr - 13 Gyr (ancient). The 5 phase model (**SFHp5**) also built-in is similar, but it splits the old phase into 1 Gyr - 5 Gyr (mid) and 5 Gyr - 9 Gyr (old). It is not really possible to break the SFH into any more independent phases than this using broad band photometry alone, but a physically motivated functional model (e.g. an exponentially decling SFR, or constant SFR) can be used using the functional interface in **SFHfunc**.
